use <../lib.scad>
$fa=1; $fs=0.2;
function ldraw_lib__3_8ring5(realsolid=false) = [
// 0 Ring  5 x 0.375
// 0 Name: 3-8ring5.dat
// 0 Author: Niels Karsdorp [nielsk]
// 0 !LDRAW_ORG Primitive UPDATE 2009-01
// 0 !LICENSE Licensed under CC BY 2.0 and CC BY 4.0 : see CAreadme.txt
// 
// 0 BFC CERTIFY CCW
  [0,"BFC","CERTIFY"],
  [0,"BFC","CCW"],
// 
// 0 !HISTORY 2009-05-02 [PTadmin] Official Update 2009-01
// 
// 
// 4 16 6 0 0 5.5434 0 2.2962 4.6195 0 1.9135 5 0 0
  [4,16,6,0,0,5.5434,0,2.2962,4.6195,0,1.9135,5,0,0],
// 4 16 5.5434 0 2.2962 4.2426 0 4.2426 3.5355 0 3.5355 4.6195 0 1.9135
  [4,16,5.5434,0,2.2962,4.2426,0,4.2426,3.5355,0,3.5355,4.6195,0,1.9135],
// 4 16 4.2426 0 4.2426 2.2962 0 5.5434 1.9135 0 4.6195 3.5355 0 3.5355
  [4,16,4.2426,0,4.2426,2.2962,0,5.5434,1.9135,0,4.6195,3.5355,0,3.5355],
// 4 16 2.2962 0 5.5434 0 0 6 0 0 5 1.9135 0 4.6195
  [4,16,2.2962,0,5.5434,0,0,6,0,0,5,1.9135,0,4.6195],
// 4 16 0 0 6 -2.2962 0 5.5434 -1.9135 0 4.6195 0 0 5
  [4,16,0,0,6,-2.2962,0,5.5434,-1.9135,0,4.6195,0,0,5],
// 4 16 -2.2962 0 5.5434 -4.2426 0 4.2426 -3.5355 0 3.5355 -1.9135 0 4.6195
  [4,16,-2.2962,0,5.5434,-4.2426,0,4.2426,-3.5355,0,3.5355,-1.9135,0,4.6195],
// 0
];
module ldraw_lib__3_8ring5(step=0, col=false, unit=2/5, alt=false, line=0.2, solid=!$preview)
    makepoly(ldraw_lib__3_8ring5(solid), step=step, col=col, unit=unit, alt=alt, line=line, solid=solid);
ldraw_lib__3_8ring5(line=0.2);