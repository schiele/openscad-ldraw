use <../../lib.scad>
function ldraw_lib__48__5_48ring39() = [
// 0 Hi-Res Ring 39 x 0.1042
// 0 Name: 48\5-48ring39.dat
// 0 Author: Magnus Forsberg [MagFors]
// 0 !LDRAW_ORG 48_Primitive UPDATE 2023-04
// 0 !LICENSE Licensed under CC BY 4.0 : see CAreadme.txt
// 
// 0 BFC CERTIFY CCW
  [0,"BFC","CERTIFY"],
  [0,"BFC","CCW"],
// 
// 0 !HISTORY 2023-08-25 [OrionP] Official Update 2023-04
// 
// 
// 4 16 40 0 0 39.656 0 5.22 38.6646 0 5.0895 39 0 0
  [4,16,40,0,0,39.656,0,5.22,38.6646,0,5.0895,39,0,0],
// 4 16 39.656 0 5.22 38.636 0 10.352 37.6701 0 10.0932 38.6646 0 5.0895
  [4,16,39.656,0,5.22,38.636,0,10.352,37.6701,0,10.0932,38.6646,0,5.0895],
// 4 16 38.636 0 10.352 36.956 0 15.308 36.0321 0 14.9253 37.6701 0 10.0932
  [4,16,38.636,0,10.352,36.956,0,15.308,36.0321,0,14.9253,37.6701,0,10.0932],
// 4 16 36.956 0 15.308 34.64 0 20 33.774 0 19.5 36.0321 0 14.9253
  [4,16,36.956,0,15.308,34.64,0,20,33.774,0,19.5,36.0321,0,14.9253],
// 4 16 34.64 0 20 31.736 0 24.352 30.9426 0 23.7432 33.774 0 19.5
  [4,16,34.64,0,20,31.736,0,24.352,30.9426,0,23.7432,33.774,0,19.5],
// 0 // Build by LDPartEditor (PrimGen 2.X)
];
module ldraw_lib__48__5_48ring39(step=0, col=false, unit=2/5, alt=false, line=0.2, solid=!$preview)
    makepoly(ldraw_lib__48__5_48ring39(), step=step, col=col, unit=unit, alt=alt, line=line, solid=solid);
ldraw_lib__48__5_48ring39(line=0.2);