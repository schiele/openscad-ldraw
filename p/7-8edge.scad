use <../lib.scad>
$fa=1; $fs=0.2;
function ldraw_lib__7_8edge(realsolid=false) = [
// 0 Circle 0.875
// 0 Name: 7-8edge.dat
// 0 Author: Mark Kennedy [mkennedy]
// 0 !LDRAW_ORG Primitive UPDATE 2009-01
// 0 !LICENSE Licensed under CC BY 2.0 and CC BY 4.0 : see CAreadme.txt
// 
// 0 BFC CERTIFY CCW
  [0,"BFC","CERTIFY"],
  [0,"BFC","CCW"],
// 
// 0 !HISTORY 2009-05-02 [PTadmin] Official Update 2009-01
// 
// 
// 2 24 1 0 0 0.9239 0 0.3827
  [2,24,1,0,0,0.9239,0,0.3827],
// 2 24 0.9239 0 0.3827 0.7071 0 0.7071
  [2,24,0.9239,0,0.3827,0.7071,0,0.7071],
// 2 24 0.7071 0 0.7071 0.3827 0 0.9239
  [2,24,0.7071,0,0.7071,0.3827,0,0.9239],
// 2 24 0.3827 0 0.9239 0 0 1
  [2,24,0.3827,0,0.9239,0,0,1],
// 2 24 0 0 1 -0.3827 0 0.9239
  [2,24,0,0,1,-0.3827,0,0.9239],
// 2 24 -0.3827 0 0.9239 -0.7071 0 0.7071
  [2,24,-0.3827,0,0.9239,-0.7071,0,0.7071],
// 2 24 -0.7071 0 0.7071 -0.9239 0 0.3827
  [2,24,-0.7071,0,0.7071,-0.9239,0,0.3827],
// 2 24 -0.9239 0 0.3827 -1 0 0
  [2,24,-0.9239,0,0.3827,-1,0,0],
// 2 24 -1 0 0 -0.9239 0 -0.3827
  [2,24,-1,0,0,-0.9239,0,-0.3827],
// 2 24 -0.9239 0 -0.3827 -0.7071 0 -0.7071
  [2,24,-0.9239,0,-0.3827,-0.7071,0,-0.7071],
// 2 24 -0.7071 0 -0.7071 -0.3827 0 -0.9239
  [2,24,-0.7071,0,-0.7071,-0.3827,0,-0.9239],
// 2 24 -0.3827 0 -0.9239 0 0 -1
  [2,24,-0.3827,0,-0.9239,0,0,-1],
// 2 24 0 0 -1 0.3827 0 -0.9239
  [2,24,0,0,-1,0.3827,0,-0.9239],
// 2 24 0.3827 0 -0.9239 0.7071 0 -0.7071
  [2,24,0.3827,0,-0.9239,0.7071,0,-0.7071],
// 0
];
module ldraw_lib__7_8edge(step=0, col=false, unit=2/5, alt=false, line=0.2, solid=!$preview)
    makepoly(ldraw_lib__7_8edge(solid), step=step, col=col, unit=unit, alt=alt, line=line, solid=solid);
ldraw_lib__7_8edge(line=0.2);