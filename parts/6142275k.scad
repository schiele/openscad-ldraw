use <../lib.scad>
use <../p/1-4chrd.scad>
use <../p/1-4cyli.scad>
use <../p/box3u12.scad>
$fa=1; $fs=0.2;
function ldraw_lib__6142275k(realsolid=false) = [
// 0 Sticker  1 x  0.8 with White Trapezoid on Black Background Left
// 0 Name: 6142275k.dat
// 0 Author: Massimo Maso [Sirio]
// 0 !LDRAW_ORG Part UPDATE 2022-06
// 0 !LICENSE Licensed under CC BY 2.0 and CC BY 4.0 : see CAreadme.txt
// 
// 0 BFC CERTIFY CCW
  [0,"BFC","CERTIFY"],
  [0,"BFC","CCW"],
// 
// 0 !KEYWORDS Audi, e-tron, quattro, R18, Set 75872, Speed Champions
// 
// 0 !HISTORY 2022-12-20 [PTadmin] Official Update 2022-06
// 
// 
// 0 // Primitives
// 
// 1 16 -6.5 0 8.5 -1.5 0 0 0 -1 0 0 0 1.5 1-4chrd.dat
  [1,16,-6.5,0,8.5,-1.5,0,0,0,-1,0,0,0,1.5, ldraw_lib__1_4chrd(realsolid)],
// 1 16 6.5 0 8.5 1.5 0 0 0 -1 0 0 0 1.5 1-4chrd.dat
  [1,16,6.5,0,8.5,1.5,0,0,0,-1,0,0,0,1.5, ldraw_lib__1_4chrd(realsolid)],
// 1 16 0 -.25 0 0 0 8 0 .25 0 -8.5 0 0 box3u12.dat
  [1,16,0,-.25,0,0,0,8,0,.25,0,-8.5,0,0, ldraw_lib__box3u12(realsolid)],
// 1 16 -6.5 0 -8.5 -1.5 0 0 0 -1 0 0 0 -1.5 1-4chrd.dat
  [1,16,-6.5,0,-8.5,-1.5,0,0,0,-1,0,0,0,-1.5, ldraw_lib__1_4chrd(realsolid)],
// 1 16 6.5 0 -8.5 1.5 0 0 0 -1 0 0 0 -1.5 1-4chrd.dat
  [1,16,6.5,0,-8.5,1.5,0,0,0,-1,0,0,0,-1.5, ldraw_lib__1_4chrd(realsolid)],
// 1 16 -6.5 0 8.5 -1.5 0 0 0 -.25 0 0 0 1.5 1-4cyli.dat
  [1,16,-6.5,0,8.5,-1.5,0,0,0,-.25,0,0,0,1.5, ldraw_lib__1_4cyli(realsolid)],
// 1 16 6.5 0 8.5 1.5 0 0 0 -.25 0 0 0 1.5 1-4cyli.dat
  [1,16,6.5,0,8.5,1.5,0,0,0,-.25,0,0,0,1.5, ldraw_lib__1_4cyli(realsolid)],
// 1 16 -6.5 0 -8.5 -1.5 0 0 0 -.25 0 0 0 -1.5 1-4cyli.dat
  [1,16,-6.5,0,-8.5,-1.5,0,0,0,-.25,0,0,0,-1.5, ldraw_lib__1_4cyli(realsolid)],
// 1 16 6.5 0 -8.5 1.5 0 0 0 -.25 0 0 0 -1.5 1-4cyli.dat
  [1,16,6.5,0,-8.5,1.5,0,0,0,-.25,0,0,0,-1.5, ldraw_lib__1_4cyli(realsolid)],
// 1 0 -6.5 -.25 8.5 -1.5 0 0 0 1 0 0 0 1.5 1-4chrd.dat
  [1,0,-6.5,-.25,8.5,-1.5,0,0,0,1,0,0,0,1.5, ldraw_lib__1_4chrd(realsolid)],
// 1 0 6.5 -.25 8.5 1.5 0 0 0 1 0 0 0 1.5 1-4chrd.dat
  [1,0,6.5,-.25,8.5,1.5,0,0,0,1,0,0,0,1.5, ldraw_lib__1_4chrd(realsolid)],
// 1 15 -6.5 -.25 -8.5 -1.5 0 0 0 1 0 0 0 -1.5 1-4chrd.dat
  [1,15,-6.5,-.25,-8.5,-1.5,0,0,0,1,0,0,0,-1.5, ldraw_lib__1_4chrd(realsolid)],
// 1 0 6.5 -.25 -8.5 1.5 0 0 0 1 0 0 0 -1.5 1-4chrd.dat
  [1,0,6.5,-.25,-8.5,1.5,0,0,0,1,0,0,0,-1.5, ldraw_lib__1_4chrd(realsolid)],
// 0 // Bottom faces
// 4 16 6.5 0 10 8 0 8.5 -8 0 8.5 -6.5 0 10
  [4,16,6.5,0,10,8,0,8.5,-8,0,8.5,-6.5,0,10],
// 4 16 6.5 0 -10 -6.5 0 -10 -8 0 -8.5 8 0 -8.5
  [4,16,6.5,0,-10,-6.5,0,-10,-8,0,-8.5,8,0,-8.5],
// 0 // Lateral faces
// 4 16 -6.5 0 10 -6.5 -.25 10 6.5 -.25 10 6.5 0 10
  [4,16,-6.5,0,10,-6.5,-.25,10,6.5,-.25,10,6.5,0,10],
// 4 16 -6.5 0 -10 6.5 0 -10 6.5 -.25 -10 -6.5 -.25 -10
  [4,16,-6.5,0,-10,6.5,0,-10,6.5,-.25,-10,-6.5,-.25,-10],
// 0 // Top faces
// 0 // Black faces
// 4 0 -6.5 -.25 10 -8 -.25 8.5 8 -.25 8.5 6.5 -.25 10
  [4,0,-6.5,-.25,10,-8,-.25,8.5,8,-.25,8.5,6.5,-.25,10],
// 4 0 -8 -.25 8.5 -8 -.25 -2.5 8 -.25 -.5 8 -.25 8.5
  [4,0,-8,-.25,8.5,-8,-.25,-2.5,8,-.25,-.5,8,-.25,8.5],
// 4 0 8 -.25 -2.75 -2.25 -.25 -10 6.5 -.25 -10 8 -.25 -8.5
  [4,0,8,-.25,-2.75,-2.25,-.25,-10,6.5,-.25,-10,8,-.25,-8.5],
// 0 // White faces
// 4 15 -8 -.25 -2.5 -2.25 -.25 -10 8 -.25 -2.75 8 -.25 -.5
  [4,15,-8,-.25,-2.5,-2.25,-.25,-10,8,-.25,-2.75,8,-.25,-.5],
// 4 15 -8 -.25 -2.5 -8 -.25 -8.5 -6.5 -.25 -10 -2.25 -.25 -10
  [4,15,-8,-.25,-2.5,-8,-.25,-8.5,-6.5,-.25,-10,-2.25,-.25,-10],
];
module ldraw_lib__6142275k(step=0, col=false, unit=2/5, alt=false, line=0.2, solid=!$preview)
    makepoly(ldraw_lib__6142275k(solid), step=step, col=col, unit=unit, alt=alt, line=line, solid=solid);
ldraw_lib__6142275k(line=0.2);