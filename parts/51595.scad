use <../lib.scad>
use <../p/1-4disc.scad>
use <s/51595s01.scad>
use <../p/stud.scad>
use <../p/stug2.scad>
use <../p/stug7.scad>
$fa=1; $fs=0.2;
function ldraw_lib__51595(realsolid=false) = [
// 0 Baseplate 16 x 16 Driveway
// 0 Name: 51595.dat
// 0 Author: Ross Crawford [rosco]
// 0 !LDRAW_ORG Part UPDATE 2010-03
// 0 !LICENSE Licensed under CC BY 2.0 and CC BY 4.0 : see CAreadme.txt
// 
// 0 BFC CERTIFY CW
  [0,"BFC","CERTIFY"],
  [0,"BFC","CW"],
// 
// 0 !HISTORY 2002-08-18 [PTadmin] Official Update 2002-04
// 0 !HISTORY 2004-09-15 [PTadmin] Official Update 2004-03
// 0 !HISTORY 2007-06-07 [PTadmin] Header formatted for Contributor Agreement
// 0 !HISTORY 2008-07-01 [PTadmin] Official Update 2008-01
// 0 !HISTORY 2010-09-12 [PTadmin] Renamed from 30225
// 0 !HISTORY 2010-11-22 [Steffen] BFCed, subfiled, used stug
// 0 !HISTORY 2010-12-31 [PTadmin] Official Update 2010-03
// 
// 
// 1 16 0 0 0 1 0 0 0 1 0 0 0 1 s\51595s01.dat
  [1,16,0,0,0,1,0,0,0,1,0,0,0,1, ldraw_lib__s__51595s01(realsolid)],
// 
// 1 16 156 0 156 4 0 0 0 1 0 0 0 4 1-4disc.dat
  [1,16,156,0,156,4,0,0,0,1,0,0,0,4, ldraw_lib__1_4disc(realsolid)],
// 4 16 156 0 160 156 0 156 -156 0 156 -156 0 160
  [4,16,156,0,160,156,0,156,-156,0,156,-156,0,160],
// 1 16 -156 0 156 -4 0 0 0 1 0 0 0 4 1-4disc.dat
  [1,16,-156,0,156,-4,0,0,0,1,0,0,0,4, ldraw_lib__1_4disc(realsolid)],
// 4 16 -156 0 -156 -160 0 -156 -160 0 156 -156 0 156
  [4,16,-156,0,-156,-160,0,-156,-160,0,156,-156,0,156],
// 1 16 -156 0 -156 -4 0 0 0 1 0 0 0 -4 1-4disc.dat
  [1,16,-156,0,-156,-4,0,0,0,1,0,0,0,-4, ldraw_lib__1_4disc(realsolid)],
// 4 16 -156 0 -160 -156 0 -156 156 0 -156 156 0 -160
  [4,16,-156,0,-160,-156,0,-156,156,0,-156,156,0,-160],
// 1 16 156 0 -156 4 0 0 0 1 0 0 0 -4 1-4disc.dat
  [1,16,156,0,-156,4,0,0,0,1,0,0,0,-4, ldraw_lib__1_4disc(realsolid)],
// 4 16 160 0 -156 156 0 -156 156 0 156 160 0 156
  [4,16,160,0,-156,156,0,-156,156,0,156,160,0,156],
// 4 16 156 0 -156 -156 0 -156 -156 0 156 156 0 156
  [4,16,156,0,-156,-156,0,-156,-156,0,156,156,0,156],
// 1 16 -90 0 -90 1 0 0 0 1 0 0 0 1 stug7.dat
  [1,16,-90,0,-90,1,0,0,0,1,0,0,0,1, ldraw_lib__stug7(realsolid)],
// 1 16 50 0 -90 1 0 0 0 1 0 0 0 1 stug7.dat
  [1,16,50,0,-90,1,0,0,0,1,0,0,0,1, ldraw_lib__stug7(realsolid)],
// 1 16 140 0 -140 1 0 0 0 1 0 0 0 1 stug2.dat
  [1,16,140,0,-140,1,0,0,0,1,0,0,0,1, ldraw_lib__stug2(realsolid)],
// 1 16 140 0 -100 1 0 0 0 1 0 0 0 1 stug2.dat
  [1,16,140,0,-100,1,0,0,0,1,0,0,0,1, ldraw_lib__stug2(realsolid)],
// 1 16 140 0 -60 1 0 0 0 1 0 0 0 1 stug2.dat
  [1,16,140,0,-60,1,0,0,0,1,0,0,0,1, ldraw_lib__stug2(realsolid)],
// 1 16 130 0 -30 1 0 0 0 1 0 0 0 1 stud.dat
  [1,16,130,0,-30,1,0,0,0,1,0,0,0,1, ldraw_lib__stud(realsolid)],
// 1 16 150 0 -30 1 0 0 0 1 0 0 0 1 stud.dat
  [1,16,150,0,-30,1,0,0,0,1,0,0,0,1, ldraw_lib__stud(realsolid)],
// 1 16 150 0 150 1 0 0 0 1 0 0 0 1 stud.dat
  [1,16,150,0,150,1,0,0,0,1,0,0,0,1, ldraw_lib__stud(realsolid)],
// 1 16 130 0 150 1 0 0 0 1 0 0 0 1 stud.dat
  [1,16,130,0,150,1,0,0,0,1,0,0,0,1, ldraw_lib__stud(realsolid)],
// 1 16 110 0 150 1 0 0 0 1 0 0 0 1 stud.dat
  [1,16,110,0,150,1,0,0,0,1,0,0,0,1, ldraw_lib__stud(realsolid)],
// 1 16 90 0 150 1 0 0 0 1 0 0 0 1 stud.dat
  [1,16,90,0,150,1,0,0,0,1,0,0,0,1, ldraw_lib__stud(realsolid)],
// 1 16 70 0 150 1 0 0 0 1 0 0 0 1 stud.dat
  [1,16,70,0,150,1,0,0,0,1,0,0,0,1, ldraw_lib__stud(realsolid)],
// 1 16 50 0 150 1 0 0 0 1 0 0 0 1 stud.dat
  [1,16,50,0,150,1,0,0,0,1,0,0,0,1, ldraw_lib__stud(realsolid)],
// 1 16 30 0 150 1 0 0 0 1 0 0 0 1 stud.dat
  [1,16,30,0,150,1,0,0,0,1,0,0,0,1, ldraw_lib__stud(realsolid)],
// 1 16 10 0 150 1 0 0 0 1 0 0 0 1 stud.dat
  [1,16,10,0,150,1,0,0,0,1,0,0,0,1, ldraw_lib__stud(realsolid)],
// 1 16 -10 0 150 1 0 0 0 1 0 0 0 1 stud.dat
  [1,16,-10,0,150,1,0,0,0,1,0,0,0,1, ldraw_lib__stud(realsolid)],
// 1 16 -30 0 150 1 0 0 0 1 0 0 0 1 stud.dat
  [1,16,-30,0,150,1,0,0,0,1,0,0,0,1, ldraw_lib__stud(realsolid)],
// 1 16 -50 0 150 1 0 0 0 1 0 0 0 1 stud.dat
  [1,16,-50,0,150,1,0,0,0,1,0,0,0,1, ldraw_lib__stud(realsolid)],
// 1 16 -70 0 150 1 0 0 0 1 0 0 0 1 stud.dat
  [1,16,-70,0,150,1,0,0,0,1,0,0,0,1, ldraw_lib__stud(realsolid)],
// 1 16 -90 0 150 1 0 0 0 1 0 0 0 1 stud.dat
  [1,16,-90,0,150,1,0,0,0,1,0,0,0,1, ldraw_lib__stud(realsolid)],
// 1 16 -110 0 150 1 0 0 0 1 0 0 0 1 stud.dat
  [1,16,-110,0,150,1,0,0,0,1,0,0,0,1, ldraw_lib__stud(realsolid)],
// 1 16 -130 0 150 1 0 0 0 1 0 0 0 1 stud.dat
  [1,16,-130,0,150,1,0,0,0,1,0,0,0,1, ldraw_lib__stud(realsolid)],
// 1 16 -150 0 150 1 0 0 0 1 0 0 0 1 stud.dat
  [1,16,-150,0,150,1,0,0,0,1,0,0,0,1, ldraw_lib__stud(realsolid)],
// 
];
module ldraw_lib__51595(step=0, col=false, unit=2/5, alt=false, line=0.2, solid=!$preview)
    makepoly(ldraw_lib__51595(solid), step=step, col=col, unit=unit, alt=alt, line=line, solid=solid);
ldraw_lib__51595(line=0.2);