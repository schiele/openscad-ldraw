use <../lib.scad>
use <2431.scad>
use <821410c.scad>
$fa=1; $fs=0.2;
function ldraw_lib__2431d01(realsolid=false) = [
// 0 ~Tile  1 x  4 with Black "POLICE" and Red Line Sticker - Obsolete
// 0 Name: 2431d01.dat
// 0 Author: Michael Heidemann [mikeheide]
// 0 !LDRAW_ORG Shortcut UPDATE 2015-02
// 0 !LICENSE Licensed under CC BY 2.0 and CC BY 4.0 : see CAreadme.txt
// 
// 0 BFC CERTIFY CCW
  [0,"BFC","CERTIFY"],
  [0,"BFC","CCW"],
// 
// 0 !KEYWORDS set 6398
// 
// 0 !HISTORY 2012-03-30 [PTadmin] Official Update 2012-01
// 0 !HISTORY 2015-11-17 [cwdee] Obsoleted due to incorrect origin
// 0 !HISTORY 2015-12-30 [PTadmin] Official Update 2015-02
// 
// 
// 1 16 0 1 0 1 0 0 0 1 0 0 0 1 2431.dat
  [1,16,0,1,0,1,0,0,0,1,0,0,0,1, ldraw_lib__2431(realsolid)],
// 1 16 0 1 0 1 0 0 0 1 0 0 0 1 821410c.dat
  [1,16,0,1,0,1,0,0,0,1,0,0,0,1, ldraw_lib__821410c(realsolid)],
];
module ldraw_lib__2431d01(step=0, col=false, unit=2/5, alt=false, line=0.2, solid=!$preview)
    makepoly(ldraw_lib__2431d01(solid), step=step, col=col, unit=unit, alt=alt, line=line, solid=solid);
ldraw_lib__2431d01(line=0.2);