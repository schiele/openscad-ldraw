use <../lib.scad>
use <../p/box4-1.scad>
use <../p/box5.scad>
use <../p/stud7.scad>
use <../p/stud8.scad>
$fa=1; $fs=0.2;
function ldraw_lib__40666(realsolid=false) = [
// 0 Duplo Plate  2 x  4
// 0 Name: 40666.dat
// 0 Author: Paul Schelleman [schellie]
// 0 !LDRAW_ORG Part UPDATE 2009-02
// 0 !LICENSE Licensed under CC BY 2.0 and CC BY 4.0 : see CAreadme.txt
// 
// 0 BFC CERTIFY CCW
  [0,"BFC","CERTIFY"],
  [0,"BFC","CCW"],
// 
// 0 !HISTORY 2009-09-03 [PTadmin] Official Update 2009-02
// 
// 
// 1 16 40 4 0 1 0 0 0 -4 0 0 0 1 stud8.dat
  [1,16,40,4,0,1,0,0,0,-4,0,0,0,1, ldraw_lib__stud8(realsolid)],
// 1 16 0 4 0 1 0 0 0 -4 0 0 0 1 stud8.dat
  [1,16,0,4,0,1,0,0,0,-4,0,0,0,1, ldraw_lib__stud8(realsolid)],
// 1 16 -40 4 0 1 0 0 0 -4 0 0 0 1 stud8.dat
  [1,16,-40,4,0,1,0,0,0,-4,0,0,0,1, ldraw_lib__stud8(realsolid)],
// 0 BFC INVERTNEXT
  [0,"BFC","INVERTNEXT"],
// 1 16 0 24 0 76 0 0 0 -20 0 0 0 36 box5.dat
  [1,16,0,24,0,76,0,0,0,-20,0,0,0,36, ldraw_lib__box5(realsolid)],
// 4 16 80 24 40 76 24 36 -76 24 36 -80 24 40
  [4,16,80,24,40,76,24,36,-76,24,36,-80,24,40],
// 4 16 -80 24 40 -76 24 36 -76 24 -36 -80 24 -40
  [4,16,-80,24,40,-76,24,36,-76,24,-36,-80,24,-40],
// 4 16 -80 24 -40 -76 24 -36 76 24 -36 80 24 -40
  [4,16,-80,24,-40,-76,24,-36,76,24,-36,80,24,-40],
// 4 16 80 24 -40 76 24 -36 76 24 36 80 24 40
  [4,16,80,24,-40,76,24,-36,76,24,36,80,24,40],
// 1 16 0 24 0 80 0 0 0 -24 0 0 0 40 box5.dat
  [1,16,0,24,0,80,0,0,0,-24,0,0,0,40, ldraw_lib__box5(realsolid)],
// 1 16 60 0 20 1 0 0 0 2.75 0 0 0 1 stud7.dat
  [1,16,60,0,20,1,0,0,0,2.75,0,0,0,1, ldraw_lib__stud7(realsolid)],
// 1 16 20 0 20 1 0 0 0 2.75 0 0 0 1 stud7.dat
  [1,16,20,0,20,1,0,0,0,2.75,0,0,0,1, ldraw_lib__stud7(realsolid)],
// 1 16 -20 0 20 1 0 0 0 2.75 0 0 0 1 stud7.dat
  [1,16,-20,0,20,1,0,0,0,2.75,0,0,0,1, ldraw_lib__stud7(realsolid)],
// 1 16 -60 0 20 1 0 0 0 2.75 0 0 0 1 stud7.dat
  [1,16,-60,0,20,1,0,0,0,2.75,0,0,0,1, ldraw_lib__stud7(realsolid)],
// 1 16 60 0 -20 1 0 0 0 2.75 0 0 0 1 stud7.dat
  [1,16,60,0,-20,1,0,0,0,2.75,0,0,0,1, ldraw_lib__stud7(realsolid)],
// 1 16 20 0 -20 1 0 0 0 2.75 0 0 0 1 stud7.dat
  [1,16,20,0,-20,1,0,0,0,2.75,0,0,0,1, ldraw_lib__stud7(realsolid)],
// 1 16 -20 0 -20 1 0 0 0 2.75 0 0 0 1 stud7.dat
  [1,16,-20,0,-20,1,0,0,0,2.75,0,0,0,1, ldraw_lib__stud7(realsolid)],
// 1 16 -60 0 -20 1 0 0 0 2.75 0 0 0 1 stud7.dat
  [1,16,-60,0,-20,1,0,0,0,2.75,0,0,0,1, ldraw_lib__stud7(realsolid)],
// 1 16 60 4 -34 1.5 0 0 0 19 0 0 0 2 box4-1.dat
  [1,16,60,4,-34,1.5,0,0,0,19,0,0,0,2, ldraw_lib__box4_1(realsolid)],
// 1 16 20 4 -34 1.5 0 0 0 19 0 0 0 2 box4-1.dat
  [1,16,20,4,-34,1.5,0,0,0,19,0,0,0,2, ldraw_lib__box4_1(realsolid)],
// 1 16 -20 4 -34 1.5 0 0 0 19 0 0 0 2 box4-1.dat
  [1,16,-20,4,-34,1.5,0,0,0,19,0,0,0,2, ldraw_lib__box4_1(realsolid)],
// 1 16 -60 4 -34 1.5 0 0 0 19 0 0 0 2 box4-1.dat
  [1,16,-60,4,-34,1.5,0,0,0,19,0,0,0,2, ldraw_lib__box4_1(realsolid)],
// 1 16 60 4 34 1.5 0 0 0 19 0 0 0 -2 box4-1.dat
  [1,16,60,4,34,1.5,0,0,0,19,0,0,0,-2, ldraw_lib__box4_1(realsolid)],
// 1 16 20 4 34 1.5 0 0 0 19 0 0 0 -2 box4-1.dat
  [1,16,20,4,34,1.5,0,0,0,19,0,0,0,-2, ldraw_lib__box4_1(realsolid)],
// 1 16 -20 4 34 1.5 0 0 0 19 0 0 0 -2 box4-1.dat
  [1,16,-20,4,34,1.5,0,0,0,19,0,0,0,-2, ldraw_lib__box4_1(realsolid)],
// 1 16 -60 4 34 1.5 0 0 0 19 0 0 0 -2 box4-1.dat
  [1,16,-60,4,34,1.5,0,0,0,19,0,0,0,-2, ldraw_lib__box4_1(realsolid)],
// 1 16 74 4 20 0 0 -2 0 19 0 -1.5 0 0 box4-1.dat
  [1,16,74,4,20,0,0,-2,0,19,0,-1.5,0,0, ldraw_lib__box4_1(realsolid)],
// 1 16 74 4 -20 0 0 -2 0 19 0 -1.5 0 0 box4-1.dat
  [1,16,74,4,-20,0,0,-2,0,19,0,-1.5,0,0, ldraw_lib__box4_1(realsolid)],
// 1 16 -74 4 20 0 0 2 0 19 0 -1.5 0 0 box4-1.dat
  [1,16,-74,4,20,0,0,2,0,19,0,-1.5,0,0, ldraw_lib__box4_1(realsolid)],
// 1 16 -74 4 -20 0 0 2 0 19 0 -1.5 0 0 box4-1.dat
  [1,16,-74,4,-20,0,0,2,0,19,0,-1.5,0,0, ldraw_lib__box4_1(realsolid)],
// 0
];
module ldraw_lib__40666(step=0, col=false, unit=2/5, alt=false, line=0.2, solid=!$preview)
    makepoly(ldraw_lib__40666(solid), step=step, col=col, unit=unit, alt=alt, line=line, solid=solid);
ldraw_lib__40666(line=0.2);