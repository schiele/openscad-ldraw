use <../lib.scad>
use <../p/4-4disc.scad>
use <../p/4-4ring1.scad>
use <../p/4-4ring2.scad>
use <../p/4-4ring3.scad>
use <../p/4-4ring4.scad>
use <../p/4-4ring5.scad>
use <s/14769s01.scad>
$fa=1; $fs=0.2;
function ldraw_lib__14769p09(realsolid=false) = [
// 0 Tile  2 x  2 Round with Round Underside Stud with Archery Target Pattern
// 0 Name: 14769p09.dat
// 0 Author: Evert-Jan Boer [ejboer]
// 0 !LDRAW_ORG Part UPDATE 2017-01
// 0 !LICENSE Licensed under CC BY 2.0 and CC BY 4.0 : see CAreadme.txt
// 
// 0 BFC CERTIFY CCW
  [0,"BFC","CERTIFY"],
  [0,"BFC","CCW"],
// 
// 0 !KEYWORDS Arrow, BrickLink 14769pb086
// 
// 0 !HISTORY 2017-12-30 [PTadmin] Official Update 2017-01
// 
// 
// 1 16 0 0 0 1 0 0 0 1 0 0 0 1 s\14769s01.dat
  [1,16,0,0,0,1,0,0,0,1,0,0,0,1, ldraw_lib__s__14769s01(realsolid)],
// 
// 1 14 0 0 0 3.3333 0 0 0 1 0 0 0 3.3333 4-4disc.dat
  [1,14,0,0,0,3.3333,0,0,0,1,0,0,0,3.3333, ldraw_lib__4_4disc(realsolid)],
// 1 16 0 0 0 3.3333 0 0 0 1 0 0 0 3.3333 4-4ring1.dat
  [1,16,0,0,0,3.3333,0,0,0,1,0,0,0,3.3333, ldraw_lib__4_4ring1(realsolid)],
// 1 4 0 0 0 3.3333 0 0 0 1 0 0 0 3.3333 4-4ring2.dat
  [1,4,0,0,0,3.3333,0,0,0,1,0,0,0,3.3333, ldraw_lib__4_4ring2(realsolid)],
// 1 16 0 0 0 3.3333 0 0 0 1 0 0 0 3.3333 4-4ring3.dat
  [1,16,0,0,0,3.3333,0,0,0,1,0,0,0,3.3333, ldraw_lib__4_4ring3(realsolid)],
// 1 1 0 0 0 3.3333 0 0 0 1 0 0 0 3.3333 4-4ring4.dat
  [1,1,0,0,0,3.3333,0,0,0,1,0,0,0,3.3333, ldraw_lib__4_4ring4(realsolid)],
// 1 16 0 0 0 3.3333 0 0 0 1 0 0 0 3.3333 4-4ring5.dat
  [1,16,0,0,0,3.3333,0,0,0,1,0,0,0,3.3333, ldraw_lib__4_4ring5(realsolid)],
];
module ldraw_lib__14769p09(step=0, col=false, unit=2/5, alt=false, line=0.2, solid=!$preview)
    makepoly(ldraw_lib__14769p09(solid), step=step, col=col, unit=unit, alt=alt, line=line, solid=solid);
ldraw_lib__14769p09(line=0.2);