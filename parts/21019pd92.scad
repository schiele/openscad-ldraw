use <../lib.scad>
use <20460pd92.scad>
use <20461pd92.scad>
use <3815b.scad>
$fa=1; $fs=0.2;
function ldraw_lib__21019pd92(realsolid=false) = [
// 0 Minifig Hips and Legs with Ruffles, Black Lower Leg and White Feet Pattern
// 0 Name: 21019pd92.dat
// 0 Author: Gerald Lasser [GeraldLasser]
// 0 !LDRAW_ORG Shortcut UPDATE 2022-05
// 0 !LICENSE Licensed under CC BY 2.0 and CC BY 4.0 : see CAreadme.txt
// 
// 0 BFC CERTIFY CCW
  [0,"BFC","CERTIFY"],
  [0,"BFC","CCW"],
// 
// 0 !KEYWORDS Bricklink 970c00pb0962, Collectible Minifigs, set 71024-2
// 0 !KEYWORDS Vintage Minnie Mouse
// 
// 0 !HISTORY 2020-12-29 [PTadmin] Official Update 2020-03
// 0 !HISTORY 2021-03-22 [cwdee] Correct BrickLink reference, add set
// 0 !HISTORY 2022-09-15 [PTadmin] Official Update 2022-05
// 
// 
// 1 16 0 0 0 1 0 0 0 1 0 0 0 1 3815b.dat
  [1,16,0,0,0,1,0,0,0,1,0,0,0,1, ldraw_lib__3815b(realsolid)],
// 1 16 0 12 0 1 0 0 0 1 0 0 0 1 20460pd92.dat
  [1,16,0,12,0,1,0,0,0,1,0,0,0,1, ldraw_lib__20460pd92(realsolid)],
// 1 16 0 12 0 1 0 0 0 1 0 0 0 1 20461pd92.dat
  [1,16,0,12,0,1,0,0,0,1,0,0,0,1, ldraw_lib__20461pd92(realsolid)],
];
module ldraw_lib__21019pd92(step=0, col=false, unit=2/5, alt=false, line=0.2, solid=!$preview)
    makepoly(ldraw_lib__21019pd92(solid), step=step, col=col, unit=unit, alt=alt, line=line, solid=solid);
ldraw_lib__21019pd92(line=0.2);