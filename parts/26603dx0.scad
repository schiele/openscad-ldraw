use <../lib.scad>
use <26603.scad>
use <6288456b.scad>
$fa=1; $fs=0.2;
function ldraw_lib__26603dx0(realsolid=false) = [
// 0 Tile  2 x  3 with White "LIVE TONIGHT" and Bright Light Yellow "PHOEBE BUFFAY" Sticker
// 0 Name: 26603dx0.dat
// 0 Author: Vincent Messenet [Cheenzo]
// 0 !LDRAW_ORG Shortcut UPDATE 2020-01
// 0 !LICENSE Licensed under CC BY 2.0 and CC BY 4.0 : see CAreadme.txt
// 
// 0 BFC CERTIFY CCW
  [0,"BFC","CERTIFY"],
  [0,"BFC","CCW"],
// 
// 0 !KEYWORDS Central, Friends, Perk, Set 21319, Smelly Cat, Stars
// 
// 0 !HISTORY 2020-06-28 [PTadmin] Official Update 2020-01
// 
// 
// 1 16 0 0 0 1 0 0 0 1 0 0 0 1 26603.dat
  [1,16,0,0,0,1,0,0,0,1,0,0,0,1, ldraw_lib__26603(realsolid)],
// 1 16 0 0 0 1 0 0 0 1 0 0 0 1 6288456b.dat
  [1,16,0,0,0,1,0,0,0,1,0,0,0,1, ldraw_lib__6288456b(realsolid)],
];
module ldraw_lib__26603dx0(step=0, col=false, unit=2/5, alt=false, line=0.2, solid=!$preview)
    makepoly(ldraw_lib__26603dx0(solid), step=step, col=col, unit=unit, alt=alt, line=line, solid=solid);
ldraw_lib__26603dx0(line=0.2);