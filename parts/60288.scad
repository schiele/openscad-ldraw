use <../lib.scad>
use <57587.scad>
function ldraw_lib__60288() = [
// 0 =Technic Dome  7 x  7 x  5
// 0 Name: 60288.dat
// 0 Author: Santeri Piippo [arezey]
// 0 !LDRAW_ORG Part Alias UPDATE 2013-02
// 0 !LICENSE Licensed under CC BY 2.0 and CC BY 4.0 : see CAreadme.txt
// 
// 0 !HELP Part 60288 is the counterpart of 57587. Visually, the two parts seem
// 0 !HELP identical. This file is provided to make it easier to locate part files
// 0 !HELP when using numbers from other sources.
// 
// 0 BFC CERTIFY CCW
  [0,"BFC","CERTIFY"],
  [0,"BFC","CCW"],
// 
// 0 !HISTORY 2010-04-05 [PTadmin] Official Update 2010-01
// 0 !HISTORY 2013-12-14 [cwdee] Add = prefix
// 0 !HISTORY 2013-12-23 [PTadmin] Official Update 2013-02
// 
// 
// 1 16 0 0 0 1 0 0 0 1 0 0 0 1 57587.dat
  [1,16,0,0,0,1,0,0,0,1,0,0,0,1, ldraw_lib__57587()],
// 
];
makepoly(ldraw_lib__60288(), line=0.2);