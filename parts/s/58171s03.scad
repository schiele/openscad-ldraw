use <../../lib.scad>
use <58171s04.scad>
function ldraw_lib__s__58171s03() = [
// 0 ~Electric Hub Mario Battery Lid Buttons
// 0 Name: s\58171s03.dat
// 0 Author: Philippe Hurbain [Philo]
// 0 !LDRAW_ORG Subpart UPDATE 2023-06
// 0 !LICENSE Licensed under CC BY 4.0 : see CAreadme.txt
// 
// 0 BFC CERTIFY CCW
  [0,"BFC","CERTIFY"],
  [0,"BFC","CCW"],
// 
// 0 !HISTORY 2023-11-19 [OrionP] Official Update 2023-06
// 
// 
// 1 16 0 67.7 0 1 0 0 0 1 0 0 0 1 s\58171s04.dat
  [1,16,0,67.7,0,1,0,0,0,1,0,0,0,1, ldraw_lib__s__58171s04()],
// 1 16 0 50.4 0 -1 0 0 0 -1 0 0 0 1 s\58171s04.dat
  [1,16,0,50.4,0,-1,0,0,0,-1,0,0,0,1, ldraw_lib__s__58171s04()],
// 4 16 11.913 80.5 7.5 -11.913 80.5 7.5 -8 73.1 7.5 8 73.1 7.5
  [4,16,11.913,80.5,7.5,-11.913,80.5,7.5,-8,73.1,7.5,8,73.1,7.5],
// 2 24 -11.913 80.5 7.5 11.913 80.5 7.5
  [2,24,-11.913,80.5,7.5,11.913,80.5,7.5],
// 2 24 -11.913 37.6 7.5 11.913 37.6 7.5
  [2,24,-11.913,37.6,7.5,11.913,37.6,7.5],
// 4 16 -8 45 7.5 -11.913 37.6 7.5 11.913 37.6 7.5 8 45 7.5
  [4,16,-8,45,7.5,-11.913,37.6,7.5,11.913,37.6,7.5,8,45,7.5],
// 4 16 -11.913 37.6 7.5 -13.658 37.6 6.289 13.658 37.6 6.289 11.913 37.6 7.5
  [4,16,-11.913,37.6,7.5,-13.658,37.6,6.289,13.658,37.6,6.289,11.913,37.6,7.5],
// 2 24 -13.658 37.6 6.289 13.658 37.6 6.289
  [2,24,-13.658,37.6,6.289,13.658,37.6,6.289],
// 0 // Button Top
// 4 16 -6.733 55.4045 0.78 6.733 55.4045 0.78 6.733 45.3955 0.78 -6.733 45.3955 0.78
  [4,16,-6.733,55.4045,0.78,6.733,55.4045,0.78,6.733,45.3955,0.78,-6.733,45.3955,0.78],
// 4 16 6.733 72.7045 0.78 6.733 62.6955 0.78 -6.733 62.6955 0.78 -6.733 72.7045 0.78
  [4,16,6.733,72.7045,0.78,6.733,62.6955,0.78,-6.733,62.6955,0.78,-6.733,72.7045,0.78],
];
module ldraw_lib__s__58171s03(step=0, col=false, unit=2/5, alt=false, line=0.2, solid=!$preview)
    makepoly(ldraw_lib__s__58171s03(), step=step, col=col, unit=unit, alt=alt, line=line, solid=solid);
ldraw_lib__s__58171s03(line=0.2);