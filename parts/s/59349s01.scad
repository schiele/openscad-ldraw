use <../../lib.scad>
use <../../p/box3u2p.scad>
use <../../p/box5.scad>
use <../../p/stud3.scad>
use <../../p/stug2-1x6.scad>
$fa=1; $fs=0.2;
function ldraw_lib__s__59349s01(realsolid=false) = [
// 0 ~Panel  1 x  6 x  5 without Front Surface
// 0 Name: s\59349s01.dat
// 0 Author: Gerald Lasser [GeraldLasser]
// 0 !LDRAW_ORG Subpart UPDATE 2020-01
// 0 !LICENSE Licensed under CC BY 2.0 and CC BY 4.0 : see CAreadme.txt
// 
// 0 BFC CERTIFY CCW
  [0,"BFC","CERTIFY"],
  [0,"BFC","CCW"],
// 
// 0 !HISTORY 2020-06-28 [PTadmin] Official Update 2020-01
// 
// 
// 0 BFC INVERTNEXT
  [0,"BFC","INVERTNEXT"],
// 1 16 0 58 10 56 0 0 0 0 54 0 -16 0 box5.dat
  [1,16,0,58,10,56,0,0,0,0,54,0,-16,0, ldraw_lib__box5(realsolid)],
// 1 16 0 120 0 0 0 -60 0 -120 0 -10 0 0 box3u2p.dat
  [1,16,0,120,0,0,0,-60,0,-120,0,-10,0,0, ldraw_lib__box3u2p(realsolid)],
// 2 24 -60 120 -10 60 120 -10
  [2,24,-60,120,-10,60,120,-10],
// 1 16 -40 116 0 -1 0 0 0 -1 0 0 0 -1 stud3.dat
  [1,16,-40,116,0,-1,0,0,0,-1,0,0,0,-1, ldraw_lib__stud3(realsolid)],
// 1 16 -20 116 0 -1 0 0 0 -1 0 0 0 -1 stud3.dat
  [1,16,-20,116,0,-1,0,0,0,-1,0,0,0,-1, ldraw_lib__stud3(realsolid)],
// 1 16 0 116 0 -1 0 0 0 -1 0 0 0 -1 stud3.dat
  [1,16,0,116,0,-1,0,0,0,-1,0,0,0,-1, ldraw_lib__stud3(realsolid)],
// 1 16 20 116 0 -1 0 0 0 -1 0 0 0 -1 stud3.dat
  [1,16,20,116,0,-1,0,0,0,-1,0,0,0,-1, ldraw_lib__stud3(realsolid)],
// 1 16 40 116 0 -1 0 0 0 -1 0 0 0 -1 stud3.dat
  [1,16,40,116,0,-1,0,0,0,-1,0,0,0,-1, ldraw_lib__stud3(realsolid)],
// 0 BFC INVERTNEXT
  [0,"BFC","INVERTNEXT"],
// 1 16 0 120 0 -56 0 0 0 -4 0 0 0 -6 box5.dat
  [1,16,0,120,0,-56,0,0,0,-4,0,0,0,-6, ldraw_lib__box5(realsolid)],
// 4 16 -60 120 -10 -56 120 -6 56 120 -6 60 120 -10
  [4,16,-60,120,-10,-56,120,-6,56,120,-6,60,120,-10],
// 4 16 60 120 -10 56 120 -6 56 120 6 60 120 10
  [4,16,60,120,-10,56,120,-6,56,120,6,60,120,10],
// 4 16 60 120 10 56 120 6 -56 120 6 -60 120 10
  [4,16,60,120,10,56,120,6,-56,120,6,-60,120,10],
// 4 16 -60 120 10 -56 120 6 -56 120 -6 -60 120 -10
  [4,16,-60,120,10,-56,120,6,-56,120,-6,-60,120,-10],
// 1 16 0 0 0 -1 0 0 0 1 0 0 0 -1 stug2-1x6.dat
  [1,16,0,0,0,-1,0,0,0,1,0,0,0,-1, ldraw_lib__stug2_1x6(realsolid)],
// 4 16 56 112 10 60 120 10 -60 120 10 -56 112 10
  [4,16,56,112,10,60,120,10,-60,120,10,-56,112,10],
// 4 16 -60 120 10 -60 0 10 -56 4 10 -56 112 10
  [4,16,-60,120,10,-60,0,10,-56,4,10,-56,112,10],
// 4 16 60 0 10 60 120 10 56 112 10 56 4 10
  [4,16,60,0,10,60,120,10,56,112,10,56,4,10],
// 4 16 -56 4 10 -60 0 10 60 0 10 56 4 10
  [4,16,-56,4,10,-60,0,10,60,0,10,56,4,10],
// 2 24 60 120 10 -60 120 10
  [2,24,60,120,10,-60,120,10],
];
module ldraw_lib__s__59349s01(step=0, col=false, unit=2/5, alt=false, line=0.2, solid=!$preview)
    makepoly(ldraw_lib__s__59349s01(solid), step=step, col=col, unit=unit, alt=alt, line=line, solid=solid);
ldraw_lib__s__59349s01(line=0.2);